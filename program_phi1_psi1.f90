...
real*8, allocatable :: ct(:), ds2(:), dt(:), diff_s2(:)
real*8, allocatable :: prob(:,:,:), cv(:,:), vbias(:)

real*8 :: bias_fact, norm, t_cv, t_sys, alpha_cv, beta_cv, addition, dummy, dum    
...
open(50,file='1_PROB_3D.dat',form='unformatted',status='replace' )
...
do k = 1, md_steps
  read(4,*)   dum, cv(2,k), cv(3,k), cv(1,k), dum, dum, dum, dum, dum 
!                             phi2, phi1, psi1
...
do k=1,md_steps
read(7,'(I10,2F16.6)') dum3, vbias(k), dum
end do



allocate(prob(nbin(1),nbin(2),nbin(3)))
norm=0.0
prob=0.0
print *, 'calculating unbiased probabilty density => free energy.dat'

DO i_md=1,md_steps
        IF((i_md.GE.t_min).AND.(i_md.LE.t_max))THEN                                      ! t_min = 1 not 0
          ...
          index3 = nint((cv(3,i_md)-grid_min(3))/grid_width(3))+1            ! pbmtd applied
           if(index1.gt.0.and.index2.gt.0.and.index3.gt.0.and.index1.le.nbin(1).and.index2.le.&
              nbin(2).and.index3.le.nbin(3))then

             i_mtd= ((i_md-1)*w_cv/w_hill) + 1    ! bias is added at 0 step

             dummy = vbias(i_md) + ct(i_mtd)      
                    
             rweight = dexp(beta_cv*dummy)
             
             prob(index1,index2,index3) = prob(index1,index2,index3) + rweight
    ...
    DO i_s1=1,nbin(1)
       DO i_s2=1,nbin(2)
          DO i_s3=1,nbin(3)
               ...
               prob(i_s1,i_s2,i_s3) = prob(i_s1,i_s2,i_s3)*(1.d0/norm)                                    
              
               WRITE(50) prob(i_s1,i_s2,i_s3)
